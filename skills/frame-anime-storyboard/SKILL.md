...
Show every available bubble pack once and recommend one. Bubble selection controls post-production only. Read the live pack's concrete `assets` and assign each text block both a broad `bubble_semantic` (`dialogue`, `thought`, `narration`, `shout`, or `sfx`) and the most appropriate concrete `bubble_asset_id`. Choose by emotion, voice source, text length, speaker count, tail direction, and scene purpose; use `bubble_intent` as a semantic fallback when useful. Leave the asset automatic only for ordinary dialogue with no better specialized match, and never choose an asset merely to create variety. Keep dialogue and narration concise and rely on the picture for information that can be shown visually. Never ask the image model to draw readable dialogue, captions, or bubbles.
